...
#welcome to CKTEMP18!!! its going to be sick!!!!


#enter this at the begining of code when working with tkinter
root = Tk()
root.title("Thermostat Keypad")
...
def temp_ent() : #button_enter - button updates stemp
	...
	print("\nNew setpoint: " + str(stemp) + "ºF please wait")
...
def set_temp() :
	print("\n\nWELCOME TO PI THERMOSTAT")
	print("Current temperatre: " + str(ftemp) + "° Fahrenheit")
	...
	while True :
		try :
			...
			if  tempdif2 < 0 :
				...
				print("Cooling mode setpoint met.")
				GPIO.output(G, False)
				GPIO.output(Y, False)
				GPIO.output(O, False)
				GPIO.output(W, False)
...
set_temp() #run set_temp function

#end program is always waiting for a keyboard "Enter" to reset program, this keeps the program in a "standby" so you can run functions.
end_program = input("\nPlese enter new setpoint in keypad, \nthen select HVAC mode using buttons:\n\nRed Button - Heatpump Heat Mode \nYellow Button - Em Heat Mode \nBlue - Heatpump Cooling Mode \n\nPress enter anytime to end program\n")
GPIO.output(G,False)
GPIO.output(Y,False)
GPIO.output(O, False)
GPIO.output(W, False)
