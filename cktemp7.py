...
print("here is your temperature differential: " + str(tempdif))

modeselection = OFF
print("Please Enter Mode Selection ")
try:
	while True:
		if (GPIO.input(HEAT) == 0) :
			print("SYS heating")
			
		elif (GPIO.input(COOLING) == 0) :
			print("sys cooling")
			break
		elif (GPIO.input(EMHEAT) == 0) :
			print("sys EMheating shiiiiiiieeeit")
			break
		else :
			print("press mode to start")
			time.sleep(1) 
...
while True :
	...
	print("current temperature: " + str(ctemp2))
	time.sleep(5)
	if tempdif2 > 0 :
